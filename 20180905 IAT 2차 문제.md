1. letterCapitalize 문제를 풀어나간 제 생각 흐름을 그대로 적어보았습니다. 
...
function letterCapitalize(str) {
    ...
    for (var i=0; i<str.length; i++) {
        ...
        if (str[i+1] === ' ' || str[i+1] === undefined) { 
            //슈도 코드엔 없었던 코드화 해주면서 추가된 undefined 조건. 문장이 끝날 경우에 push 처리를 안 해준다는 게 생각나서.
...
(2) ["Hello", " world"]
// 공백world 부분이 제대로 작동하지 않음. 왜냐하면 공백이 하나만 있을 경우를 생각하지 못했음. 하나일 경우에 바로 뒤에 문자열이 나오기 때문에 push가 실행이 안 되고 계속 진행됨.

// 어떻게든 공백이 하나일 경우에 잘라줘야 하는데, 이 경우엔 스스로(str[i])가 공백이고, 다음(str[i+1])이 문자일 경우를 넣어주면 되지 않을까?
...
//역시 이 문제도 머리로 생각했던 부분과 실제로 구현되면서 좀 달라진 부분들이 존재합니다. count 부분이 추가되었죠. fn() 첫 시작이 0부터라는 걸 간과했더라구요. 어떻게 해결할까 하다가, index 개념의 카운트를 넣어주면 간단하지 않을까? 싶었습니다. 막 깔끔하진 않지만 어쨌든 해결은 됐네요.
```
